...
class data_loader:
    def __init__(self, task_name, path='data', batch_size=32, none_mask=True):
        ...
        self.X, self.y = torch.tensor(self.X).cuda(), torch.tensor(self.y).cuda()
...
WINDOW_SIZE = 4
PATIENCE = 10
def train_validate_model(model, train_generator, val_generator, optim, model_name, config, generate_every=1e3, num_batches=1e4, verbose=True, overfit_stop=True, print_file=None, tag='', log_path='logs/', head_start=15):
