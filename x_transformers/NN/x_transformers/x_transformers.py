...
class TransformerWrapper(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.pos_emb = AbsolutePositionalEmbedding(emb_dim, max_seq_len+default(num_memory_tokens, 0)) if (use_pos_emb and not attn_layers.has_pos_emb) else always(0)
    ...
    def forward(
        ...
        **kwargs
        ...
        if n > self.max_seq_len:
            ...
            chunk_mask = mask[:, -self.max_seq_len:] if exists(mask) else mask
            rest = x[:, :-self.max_seq_len]
            rest_mask = mask[:, :-self.max_seq_len] if exists(mask) else mask
...
class XTransformer(nn.Module):
    ...
    def generate(self, seq_in, seq_out_start, seq_len, src_mask = None, **kwargs):
        ...
        if src_mask is not None:
            d = encodings.shape[1] - src_mask.shape[1]
            if d > 0:
                context_mask = torch.cat((torch.ones((encodings.shape[0], d)).to(device=src_mask.device), src_mask), dim=1).bool()
            else:
                context_mask = src_mask[:, -d:]
        else:
            context_mask = None
        return self.decoder.generate(seq_out_start, seq_len, context = encodings, context_mask = context_mask, **kwargs)
    ...
    def forward(self, src, tgt, src_mask = None, tgt_mask = None):
        ...
        if src_mask is not None:
            ...
            if d > 0:
                context_mask = torch.cat((torch.ones((src.shape[0], d)).to(device=src_mask.device), src_mask), dim=1).bool()
            else:
                context_mask = src_mask[:, -d:]
        else: 
            context_mask = None
...
class XTransformer_XL(XTransformer):
    def __init__(self, enc_max_mem_len, **kwargs):
        super().__init__(**kwargs)
        self.encoder.max_mem_len = enc_max_mem_len

        
    def encode(self, src, mask=None, return_embeddings=True):
        if src.shape[1] > self.encoder.max_seq_len:
            chunk = src[:, -self.encoder.max_seq_len:]
            rest = src[:, :-self.encoder.max_seq_len]
            enc_rest, mems = self.encode(rest.detach(), mask = mask, return_embeddings = return_embeddings)
            enc, mems = self.encoder(chunk, mask = mask, return_embeddings = True, return_mems = True, mems=mems)
        else:
            enc, mems = self.encoder(src, mask = mask, return_embeddings = True, return_mems = True)
        return enc, mems

    def forward(self, src, tgt, src_mask = None, tgt_mask = None):
        enc, mems = self.encode(src, src_mask)
